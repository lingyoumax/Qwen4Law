...
## Reward Model
### 数据集
数据生成：使用微调Embedding Model的数据集中的(doc)数据，基于qwen3-235b-a22b-instruct-2507模型使用self instruct方法自动化生成(query, doc, answer_good)三元组。同时，使用经过SFT微调后的LLM生成的回答作为answer_bad。

数据示例：

用户问题：我在甘南藏族自治州工作，公司要和我解除劳动合同，这种情况应该依据哪些地方法规来处理？

| | answer_bad| answer_good|
|---|---|---|
| 回答| 根据《甘肃省甘南藏族自治州自治条例》第一条的规定，甘南藏族自治州的自治条例是依据《中华人民共和国宪法》《中华人民共和国民族区域自治法》《甘肃省实施〈中华人民共和国民族区域自治法〉若干规定》及相关法律法规制定的，适用于本州的经济、政治、文化和社会活动。在处理劳动合同解除等具体事务时，应结合本州实际情况和相关法律法规执行。 | 公司解除合同必须有合法理由并书面说明依据。如在试用期解除，须证明你“不符合录用条件”，且该条件需在入职时明确告知并有书面记录。正式员工解除则需符合法定情形，如严重违纪、不能胜任工作等，且程序合法。\n建议保存劳动合同、岗位职责、考勤记录、解除通知等证据。若公司无合法理由解除，可向当地劳动争议仲裁委员会申请仲裁，要求恢复劳动关系或支付赔偿金。|
|信息准确性	|模糊表述（“可能有关系”“大概按劳动法”），无具体依据	|准确引用《自治条例》第一条，明确地方法规定位，但未落地到 “劳动合同解除”	|既覆盖国家《劳动合同法》核心条款（试用期 / 正式员工解除条件），又给出可操作的证据保存、维权路径|
|用户价值	|解答了 “地方法规依据” 的疑问，但未解决 “如何处理解除纠纷” 的核心诉求|	直接回应 “公司解除是否合法”“该怎么做”，覆盖用户从 “判断” 到 “行动” 的全需求|
|专业性	|有法规引用，但仅停留在 “宏观依据”，无具体应用场景落地	|结合劳动纠纷场景，精准拆解法律条款 + 实操步骤，专业性强|
...
- 总损失：  
  $Loss = Loss^{good} + Loss^{bad}=-E[\log(r^{good}*(1-r^{bad}))]$
...
- $HPC = E[I_{R^+}(r^{good}-r^{median}) \cdot I_{R^+}(r^{median}-r^{bad})]$
...
|QLoRA|0.9925| 0.9748520718435771| 0.0857534628991597 |
...
- 为什么在强化学习中，需要对策略的更新幅度加以限制？
  ...
  - $Var_{x\sim p}[x]=E_{x\sim p}[x^2]-(E_{x\sim p}[x])^2$，同时$Var_{x\sim q}[\frac{p(x)}{q(x)}x]=E_{x\sim q}[(\frac{p(x)}{q(x)}x)^2]-(E_{x\sim q}[\frac{p(x)}{q(x)}x])^2=E_{x\sim p}[\frac{p(x)}{q(x)}x^2]-(E_{x\sim p}[x])^2$。可以看出，当概率分布$p$和$q$差别过大时，采样的分布的方差与原分布的方差差距会变大。这也就要求我们必须要限制策略每次更新的幅度，不然采样的数据无法使用。
...
$\because R_{\theta}=E_{\tau \sim P_{\theta}}[R_{\theta}(\tau)]=\sum_{n_{\tau}}R(\tau)P_{\theta}(\tau)$
...
$\therefore \ln P_{\theta}(\tau)=\ln \rho (s_1)+\sum_{i=1}^{l_{tau}-1}\ln \pi_{\theta}(a_i|s_i)+\sum_{i=1}^{l_{\tau}-1}\ln p(s_{i+1}|s_i,a_i)$

$\therefore \nabla \ln P_{\theta}(\tau)=\sum_{i=1}^{l_{tau}-1}\nabla \ln \pi_{\theta}(a_i|s_i)$
...
- 今天在设计微调奖励模型的损失函数，初始时计划将损失函数设计成 $-E[\log(sigmoid (r^{+}-r^{-}))]$ ，但是我们的 $r^+-r_-$ 的取值范围是 $[-1,1]$ ，经过 $sigmoid$ 函数之后会进一步压缩范围，所以改成 $-E[\log(sigmoid(s^+-s^-))]$ 。但是这样会带来一个新的问题，那就是 $s^+$ 可能和 $s^-$ 同时下降，但是 $s^-$ 下降的幅度更大，此时损失函数也会下降，所以我们不能只关注两者的差值，而应该分别独立观察它们的值。在实验过后，还发现了一个问题，我们的损失函数设计是针对的 $s^+$ 和 $s^-$ 之间的差值，将其转换为 $r^+$ 和 $r^-$ 之后，虽然模型在HPC和MD两个指标都有提升，但是Disp指标上下降明显。此时将损失函数设置为这时想到了交叉熵函数 $-(y^{\ast} \log (y)+(1-y^{\ast})\log (1-y))$ 。分别将 $r^+$ 和 $r^-$ 的计算方式代入可得 $-(\log (sigmoid(s^+))+\log (1-sigmoid(s^-))) $，也就是 $-\log (sigmoid(s^+)(1-sigmoid(s^-)))$ 。
...
- 之前设计奖励模型的训练数据集时，是使用self_instuct生成answer_good和answer_bad，用sft之后的llm生成的回答作为answer_median，而损失函数如下:
  - 因为 $r = \frac{e^{l_{yes}}}{e^{l_{yes}}+e^{l_{no}}}= \frac{1}{1+e^{-(l_{yes}-l_{no})}}= \sigma(l_{yes}-l_{no})$
  - 令 $s = l_{yes} - l_{no}$  
  所以 $r = \sigma(s)$
  ...
  但是很难收敛，所以舍弃掉answer_median，直接将sft之后模型的输出作为answer_bad。
